...
public class TreasureHuntActivity
    extends CardboardActivity implements CardboardView.StereoRenderer, SurfaceTexture.OnFrameAvailableListener  {
  ...
  public void onSurfaceCreated(EGLConfig config) {
    ...
    cubeFoundColors.put(WorldLayoutData.FLOATING_CANVAS_FOUND_COLORS);
    cubeFoundColors.position(0);

    ByteBuffer bbNormals = ByteBuffer.allocateDirect(WorldLayoutData.FLOATING_CANVAS_NORMALS.length * 4);
    bbNormals.order(ByteOrder.nativeOrder());
    cubeNormals = bbNormals.asFloatBuffer();
    cubeNormals.put(WorldLayoutData.FLOATING_CANVAS_NORMALS);
    ...
    new Thread(
            new Runnable() {
              ...
              public void run() {
                ...
                mSurface.setOnFrameAvailableListener(TreasureHuntActivity.this);

//                Surface surface = new Surface(mSurface);
                try {
                  myCamera.setPreviewTexture(mSurface);
                } catch (IOException e) {
                  e.printStackTrace();
                }
//                surface.release();
...
/* CARDBOARD GL PROGRAM VARIABLES ANALOGOUS TO THOSE OF VIDEO SURFACEVIEW */ //just a hunch
    Log.e(TAG,"drawing cam");
    int mProgram = floorProgram;
    int maPositionHandle=floorPositionParam;
    int maTextureHandle=floorColorParam;
    int muMVPMatrixHandle=floorModelViewProjectionParam;
    int muSTMatrixHandle=floorModelViewParam;
